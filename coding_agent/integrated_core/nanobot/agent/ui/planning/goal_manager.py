...
# Suppress LiteLLM remote cost map failure warnings
...
class GoalManager:
    ...
    def __init__(self, model_name: str = "gemini/gemini-2.5-flash"):
        ...
        self.semantic_replan_required = False
        self.loop_count_tracker = {} # target -> count
    ...
    def replan_recovery(self, failed_action: Dict[str, Any], state_after: str, vision_analyzer: Any, is_loop: bool = False):
        """
        Generates and inserts recovery steps. If is_loop is True, it triggers 
        a semantic replan for the main objective.
        """
        print(f"[GoalManager] Action failed: {failed_action['action']} (Loop: {is_loop}). Initiating recovery...")
        
        if is_loop:
            print("[GoalManager] CRITICAL: Loop detected. Requesting semantic replan of main objective.")
            self.semantic_replan_required = True
            return
        ...
        if recovery_steps:
            ...
            print(f"[GoalManager] Injected {len(recovery_steps)} recovery tasks.")
        else:
            print("[GoalManager] Could not generate local recovery. Scaling to semantic replan.")
            self.semantic_replan_required = True
