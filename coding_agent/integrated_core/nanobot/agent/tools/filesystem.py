    ...
    async def execute(self, path: str, **kwargs: Any) -> str:
        try:
            ...
            if len(content) > self._MAX_CHARS:
                return content[: self._MAX_CHARS] + f"\n\n... (truncated — file is {len(content):,} chars, limit {self._MAX_CHARS:,})" # type: ignore
            return content
...
class EditFileTool(Tool):
    ...
    def _not_found_message(old_text: str, content: str, path: str) -> str:
        ...
        for i in range(max(1, len(lines) - window + 1)):
            ratio = difflib.SequenceMatcher(None, old_lines, lines[i : i + window]).ratio() # type: ignore
            if ratio > best_ratio:
        ...
        if best_ratio > 0.5:
            diff = "\n".join(
                difflib.unified_diff(
                    old_lines,
                    lines[best_start : best_start + window], # type: ignore
                    fromfile="old_text (provided)",
                    tofile=f"{path} (actual, line {best_start + 1})", # type: ignore
                    lineterm="",
                )
            )
