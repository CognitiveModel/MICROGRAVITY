...
from coding_agent.ui_agent.utils.math_utils import generate_human_like_path # type: ignore
...
class MouseController:
    ...
    Integrates ScreenGeometry for coordinate validation and CursorMonitor for hover feedback.
    """
    def __init__(self, base_speed: float = 1.0, screen_geometry=None, cursor_monitor=None):
        self.base_speed = base_speed  # Multiplier for movement durations
        self.screen_geometry = screen_geometry  # ScreenGeometry instance for coord bounds
        self.cursor_monitor = cursor_monitor    # CursorMonitor instance for hover validation
    ...
    def move_to(self, x: int, y: int, duration: Optional[float] = None, human_like: bool = True):
        ...
        Validates coordinates against screen bounds if ScreenGeometry is available.
        """
        # Coordinate validation and strict clamping
        if self.screen_geometry:
            old_x, old_y = x, y
            x, y = self.screen_geometry.clamp_to_screen(x, y)
            if x != old_x or y != old_y:
                print(f"[MouseController] Clamped out-of-bounds ({old_x}, {old_y}) -> ({x}, {y})")
        
        start_pos = pyautogui.position()
        end_pos = (x, y)
        
        # Distance determines base duration if not provided
        dist = ((end_pos[0] - start_pos[0])**2 + (end_pos[1] - start_pos[1])**2)**0.5 # type: ignore
        
        if duration is None:
            # Base logic: faster for long distances, snappy for precise movements
            duration_calc = max(0.05, min(0.6, dist / 2500.0))
    ...
    def double_click(self, button: str = 'left'):
        ...
        self._sleep_random(0.05, 0.02)
        self.click(button)

    def right_click(self):
        """Simulates a human-like right click (context menu trigger)."""
    ...
    def move_and_click(self, x: int, y: int, button: str = 'left', human_like: bool = True,
        ...
        If cursor_monitor and expected_element_type are available, validates
        that cursor type matches expectations before clicking.
        
        Returns:
            dict with 'cursor_valid' key if cursor validation was performed
        """
        self.move_to(x, y, human_like=human_like)
        # Very brief pause before clicking for snappiness
        self._sleep_random(0.05)
        
        # Cursor-based hover validation
        cursor_info = {}
        if self.cursor_monitor and expected_element_type:
            cursor_valid = self.cursor_monitor.validate_hover(expected_element_type)
            cursor_type = self.cursor_monitor.get_cursor_type()
            cursor_info = {
                'cursor_valid': cursor_valid,
                'cursor_type': cursor_type.name,
                'expected_element': expected_element_type,
            }
            if not cursor_valid:
                print(f"[MouseController] WARNING: Cursor mismatch at ({x}, {y}). "
                      f"Got {cursor_type.name}, expected type for '{expected_element_type}'")
        elif self.cursor_monitor:
            # No expected type, but still record what cursor we're seeing
            cursor_type = self.cursor_monitor.get_cursor_type()
    ...
    def execute_blueprint(self, blueprint: ActionBlueprint):
        ...
        if blueprint.source_x is not None and blueprint.source_y is not None:
            self.move_to(blueprint.source_x, blueprint.source_y, human_like=True) # type: ignore
            self._sleep_random(0.3)
        ...
        if blueprint.action_type == 'click':
            ...
            pyautogui.moveTo(target_x, target_y, duration=exec_duration, tween=easing) # type: ignore
