...
from coding_agent.ui_agent.perception.screen import ScreenObserver # type: ignore
from coding_agent.ui_agent.perception.spatial_understanding.gemini_service import GeminiSpatialService # type: ignore
...
class TeachModeObserver:
    ...
    def __init__(self):
        ...
        self.gemini = GeminiSpatialService(api_key=self._config.providers.gemini.api_key)
    ...
    def start_recording(self):
        ...
        # Start Vision Buffer
        self.capture_thread = threading.Thread(target=self._continuous_capture, daemon=True) # type: ignore
        if self.capture_thread:
            self.capture_thread.start() # type: ignore
        ...
        if self.mouse_listener:
            self.mouse_listener.start() # type: ignore
        if self.keyboard_listener:
            self.keyboard_listener.start() # type: ignore
    ...
    def stop_recording(self):
        ...
        self.is_recording = False
        if self.mouse_listener:
            self.mouse_listener.stop() # type: ignore
        if self.keyboard_listener:
            self.keyboard_listener.stop() # type: ignore
        if self.capture_thread:
            self.capture_thread.join() # type: ignore
