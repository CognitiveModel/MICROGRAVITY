...
from coding_agent.ui_agent.perception.spatial_understanding.spatial_tool import SpatialUnderstandingTool

def main():
    from nanobot.config.loader import load_config
    # Make sure we have an API key, we will pull from config
    api_key = load_config().providers.gemini.api_key
    if not api_key:
        print("Please set gemini.apiKey in config.json to test the spatial logic.")
        return
