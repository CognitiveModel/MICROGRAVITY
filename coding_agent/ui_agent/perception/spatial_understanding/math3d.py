...
def generate_box_corners(size: List[float]) -> np.ndarray:
    ...
    hx, hy, hz = [s / 2.0 for s in size] # type: ignore
    
    corners = []
    for x in [-hx, hx]: # type: ignore
        for y in [-hy, hy]: # type: ignore
            for z in [-hz, hz]: # type: ignore
...
def project_3d_bounding_box(box: Dict[str, Any], width: float, height: float, fov: float) -> Dict[str, Any]:
    ...
    top_vertices = vertices_2d[0:4] # type: ignore
    bottom_vertices = vertices_2d[4:8] # type: ignore
