...
from coding_agent.ui_agent.planning.loop_sentinel import LoopSentinel, LoopSeverity, LoopRecoveryEngine # type: ignore
...
### TANDEM BROWSER-UI COORDINATION (CRITICAL):
1. **Tool Synergy**: You and the `browser_tool` are a tag-team. 
2. **When to Delegate**: Use `{"action": "delegate_to_browser_tool", "objective": "..."}` for complex site-specific tasks (e.g. "Find the user's latest post and reply with 'Hi'", "Scrape contact info"). The browser tool has deep DOM access.
3. **When to Stay in UI Agent**: Use your GUI actions (click, type) for:
   - Navigating *to* the site (typing in address bar).
   - Managing browser tabs and windows.
   - Interacting with NON-BROWSER apps (Telegram, Notepad, CMD).
   - Cross-app workflows (e.g. "Copy text from Notepad and paste into Chrome").
4. **General Desktop Scope**: You are NOT restricted to browsers. You are a full OS Automation Agent. Use CMD, File Explorer, and Desktop apps whenever needed.
...
Your response MUST be a single JSON object with these fields:
{
  ...
  "environmental_observations": [{"category": "user_account|device_os|user_preference", "detail": "e.g., test@gmail.com is signed in"}],
  "action": "<action type>",
...
> [!TIP] OPPORTUNISTIC CONTEXT EXTRACTION
> If you notice important, stable environmental details (like which user accounts are signed in, the operating system, or specific user preferences), you MUST extract them into the `environmental_observations` array. This helps the agent learn about the user over time. If nothing new or notable is seen, provide an empty list `[]`.

> [!CAUTION] STRICT GOAL VALIDATION
> You must ONLY set `"goal_complete": true` if you visually verify the EXACT final result requested by the user has been fulfilled on screen (e.g., if asked for an Excel report, you must see the data inside Excel).
> Do NOT set `"goal_complete": true` just because you successfully opened an application or saw a message about delegation.
...
"""
    ...
    def __init__(self, model_name: str = "models/gemini-2.5-flash", situational_awareness=None,
        ...
        if not api_key:
            raise ValueError("GEMINI_API_KEY not configured in coding_agent/utils/config.py")
    ...
    def decide_next_step(self, screenshot_path: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"[AgenticPlanner] Context (safe):\n{context.encode('ascii', 'replace').decode('ascii')}")
        ...
        try:
            ...
            # Process opportunistic context facts
            if "environmental_observations" in decision and self.experiential_memory:
                for obs in decision.get("environmental_observations", []):
                    cat = obs.get("category")
                    detail = obs.get("detail")
                    if cat == "user_account" and detail:
                        # Append as a known account fact
                        account_id = f"acct_{int(time.time())}"
                        self.experiential_memory.user_profile.known_accounts[account_id] = {"detail": detail}
                        print(f"[AgenticPlanner] Learned User Account: {detail}")
                    elif cat == "device_os" and detail:
                        self.experiential_memory.device_profile.os_info = detail
                        print(f"[AgenticPlanner] Learned Device OS: {detail}")
                    elif cat == "user_preference" and detail:
                        if detail not in self.experiential_memory.user_profile.assumptions:
                            self.experiential_memory.user_profile.assumptions.append(detail)
                        print(f"[AgenticPlanner] Learned User Preference: {detail}")
        ...
        try:
            ...
            msg = f"[AgenticPlanner] Decision: {decision.get('action')} - {decision.get('reasoning', '')}"
